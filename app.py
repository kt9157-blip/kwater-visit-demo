...
- 부서 사용자 계정 신청 / 관리자 승인
- 방문 사전신청 / 관리자 승인·반려
- 초소·안내데스크 모니터링 및 입문·퇴문 처리
- 신청자·관리자·초소·안내데스크 알림 팝업
- 새로고침 후 로그인 유지(auth_token 기반 7일 세션)
...
def id_pk_sql() -> str:
    # SQLite는 INTEGER PRIMARY KEY가 자동 증가 rowid로 작동합니다.
    # PostgreSQL 등은 IDENTITY 문법을 사용합니다.
    if db_dialect() == "sqlite":
        return "INTEGER PRIMARY KEY"
    return "INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"


def get_last_insert_id(table_name: str) -> Optional[int]:
    if db_dialect() == "sqlite":
        row = fetch_one("SELECT last_insert_rowid() AS id")
    else:
        # execute()가 연결을 새로 여는 구조에서는 RETURNING이 가장 안전하지만,
        # 기존 구조를 최대한 유지하기 위해 created_at 기준 최신값을 보조로 사용합니다.
        row = fetch_one(f"SELECT id FROM {table_name} ORDER BY id DESC LIMIT 1")
...
def create_default_admins() -> None:
    """기본 관리자 계정을 안전하게 생성합니다."""
    for username, password, name, dept in DEFAULT_ADMINS:
        ...
        try:
            ...
            # Streamlit Cloud 재시작·동시 실행 중 중복 생성되면 안전하게 무시합니다.
            pass
...
def restore_login_from_token() -> None:
    """새로고침 후 URL의 auth_token으로 로그인 상태를 복구합니다."""
...
def create_visit_request_notifications(visit_id: Optional[int], user: Dict[str, Any], visitor_name: str, visit_date: str) -> None:
    """방문 신청 접수 직후 역할별 알림을 생성합니다."""
    title = "새 방문 신청 접수"
    message = f"{user['department']} {user['name']}님이 {visitor_name} 방문 신청을 등록했습니다. 방문일자: {visit_date}"
    ...
    create_notification(
        title="방문 신청 접수 완료",
        message=f"{visitor_name} 방문 신청이 정상적으로 접수되었습니다. 관리자 승인 후 모니터링 화면에 표시됩니다.",
        target_role="user",
    ...
    for location in MONITOR_LOCATIONS:
        create_notification(
            title="방문 예정 신청 알림",
            message=f"{visitor_name} 방문 신청이 접수되었습니다. 승인 후 {location} 모니터링 대상에 표시됩니다.",
            target_role="monitor",
...
def create_visit_status_notification(visit: Dict[str, Any], status: str, memo: str = "") -> None:
    label = STATUS_LABELS.get(status, status)
    message = f"{visit['visitor_name']} 방문 신청이 '{label}' 처리되었습니다."
    if memo.strip():
        message += f" 관리자 메모: {memo.strip()}"

    create_notification(
        title=f"방문 신청 {label}",
        message=message,
        target_role="user",
        target_user=visit["request_user"],
        visit_id=visit["id"],
    )


def get_unread_notification_for_current_context() -> Optional[Dict[str, Any]]:
    user = require_login()
    ...
    if st.session_state.get("monitor_ok"):
        location = st.session_state.get("monitor_location")
        return fetch_one("""
        SELECT *
        FROM notifications
        WHERE target_role = 'monitor'
          AND target_location = :location
          AND is_read = 0
        ORDER BY id ASC
        LIMIT 1
        """, {"location": location})
...
def show_notification_dialog(notification: Dict[str, Any]) -> None:
    @st.dialog("새 알림")
    def _dialog() -> None:
        st.subheader(notification["title"])
        st.write(notification.get("message") or "")
        st.caption(f"알림 시각: {notification.get('created_at') or '-'}")
        ...
        with col2:
            if st.button("나중에 보기", use_container_width=True):
                # 읽음 처리하지 않고 팝업만 닫기 위한 임시 세션 기록
                st.session_state["skip_notification_once"] = notification["id"]
                st.rerun()
...
def notification_checker() -> None:
    """현재 접속 상태에 맞는 미확인 알림 1건을 팝업으로 표시합니다."""
    notification = get_unread_notification_for_current_context()
    if not notification:
        return

    if st.session_state.get("skip_notification_once") == notification["id"]:
        return

    st.toast("새 알림이 있습니다.", icon="🔔")
    show_notification_dialog(notification)


def render_notification_checker() -> None:
    """Streamlit 버전에 따라 알림 체크를 실행합니다."""
    # st.dialog와 st.toast가 없는 낮은 버전에서는 알림 팝업을 생략합니다.
    if not hasattr(st, "dialog") or not hasattr(st, "toast"):
        return

    # fragment가 있으면 10초마다 알림을 확인합니다.
...
def login(username: str, password: str) -> Optional[Dict[str, Any]]:
    ...
    if user["status"] != "approved":
        return {
            "error": f"계정 상태가 '{STATUS_LABELS.get(user['status'], user['status'])}'입니다.\n관리자 승인 후 이용 가능합니다."
        }
    ...
    execute(
        "UPDATE users SET last_login_at=:t WHERE username=:u",
        {"t": now_str(), "u": username.strip()},
    )
...
def header() -> None:
    st.markdown(
        ...
        <div class="main-title">
            <h1>{APP_TITLE}</h1>
            <p>{APP_SUBTITLE}</p>
            <p>운영 구조: Streamlit 웹앱 + 공통 DB 저장 · 사내 노트북/고정PC/클라우드 서버 배포 가능</p>
        </div>
        """,
...
def enable_auto_refresh() -> None:
    if st.sidebar.checkbox(
        "자동 새로고침",
        value=False,
        help=f"{AUTO_REFRESH_SECONDS}초 간격으로 화면을 갱신합니다.",
    ):
    ...
    if pu or pv:
        st.markdown(
            ...
            <div class="notice-box">
                <b>관리자 확인 필요</b><br>
                승인 대기 계정 {pu}건 · 승인 대기 방문신청 {pv}건이 있습니다.
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with col1:
        ...
        if submitted:
            ...
            if result and "error" in result:
                ...
                token = create_login_token(int(result["id"]))
        ...
        st.markdown(
            """
            #### 초기 관리자 계정
            - admin01 / kwater1234 / 비상계획처
            - admin02 / kwater1234 / 정보보안처
            ...
            if not all([name.strip(), username.strip(), password]):
                ...
                try:
                    ...
                    add_event(
                        "user_request",
                        "신규 계정신청",
                        f"{department} / {name} / {username}",
                    )
                    create_notification(
                        title="신규 계정신청",
                        message=f"{department} {name}님의 계정 신청이 접수되었습니다.",
                        target_role="admin",
                    )
                    st.success("계정신청이 접수되었습니다.\n관리자 승인 후 로그인할 수 있습니다.")
                except IntegrityError:
                    st.error("이미 사용 중인 아이디입니다.")
...
def page_dashboard() -> None:
    ...
    metrics = {
        ...
        "오늘 승인 방문": fetch_one(
            "SELECT COUNT(*) AS cnt FROM visits WHERE visit_date=:d AND status='approved'",
            {"d": today},
        )["cnt"],
        "오늘 입문 완료": fetch_one(
            "SELECT COUNT(*) AS cnt FROM visits WHERE visit_date=:d AND entry_status='entered'",
            {"d": today},
        )["cnt"],
    }
...
def page_user_approval() -> None:
    ...
    for r in rows:
        with st.expander(
            f"[{STATUS_LABELS.get(r['status'], r['status'])}] {r['department']} · {r['name']} ({r['username']})",
...
# 페이지: 방문신청
# -----------------------------
def page_visit_request() -> None:
    user = st.session_state["user"]

    st.subheader("방문 사전신청 등록")
    st.caption("승인된 부서 사용자만 방문신청을 등록할 수 있습니다.")

    if st.session_state.pop("visit_submit_done", False):
        st.success("방문신청이 정상적으로 등록되었습니다.")
    ...
    if submitted:
        ...
        add_event(
            "visit_request",
            "신규 방문신청",
            f"{user['department']} / {visitor_name} / {visit_date.isoformat()}",
        )
        create_visit_request_notifications(
            visit_id=visit_id,
            user=user,
            visitor_name=visitor_name.strip(),
            visit_date=visit_date.isoformat(),
        ...
        st.session_state["visit_submit_done"] = True
        st.rerun()


def page_my_visits() -> None:
    user = st.session_state["user"]
...
def page_visit_approval() -> None:
    ...
    status_filter = st.selectbox(
        ...
        format_func=lambda x: "전체" if x == "all" else STATUS_LABELS.get(x, x),
    )
    ...
    if status_filter == "all":
        ...
        rows = fetch_all(
            "SELECT * FROM visits WHERE status=:s ORDER BY id DESC LIMIT 200",
            {"s": status_filter},
        )
    ...
    for r in rows:
        title = (
            ...
            f"{r['visit_date']} {r['visit_time']} · {r['visitor_name']} · {r['target_department']}"
        )
            ...
            if r["status"] == "pending":
                ...
                with c1:
                    if st.button("방문 승인", key=f"approve_visit_{r['id']}", use_container_width=True):
                        ...
                        create_visit_status_notification(r, "approved", admin_memo)
                        st.success("승인되었습니다.")
                        st.rerun()
                ...
                with c2:
                    if st.button("방문 반려", key=f"reject_visit_{r['id']}", use_container_width=True):
                        ...
                        create_visit_status_notification(r, "rejected", admin_memo)
                        st.warning("반려 처리되었습니다.")
                        st.rerun()
...
def page_monitor() -> None:
    ...
    if not st.session_state.get("monitor_ok"):
        ...
        with c2:
            st.markdown(
                """
                #### 모니터링 화면 용도
                - 승인된 방문자를 조회하고 입문/퇴문 처리를 합니다.
                - 기본 접속 코드는 `monitor1234` 입니다.
                - 운영 전 반드시 환경변수 `MONITOR_CODE`로 변경하세요.
                """,
                unsafe_allow_html=True,
            )
        return
    ...
    WHERE status='approved'
      AND visit_date=:d
    """
    params: Dict[str, Any] = {"d": filter_date.isoformat()}

    if keyword.strip():
        sql += """
        AND (
            visitor_name LIKE :kw
            OR visitor_org LIKE :kw
            OR target_department LIKE :kw
            OR vehicle_no LIKE :kw
            OR request_department LIKE :kw
        )
        """
...
# 페이지: 데이터 관리
# -----------------------------
def page_data_admin() -> None:
    st.subheader("데이터 백업/조회")
    admin_notification()

    tab1, tab2, tab3, tab4 = st.tabs(["방문신청 데이터", "사용자 데이터", "이벤트 로그", "알림 로그"])
    ...
    with tab4:
        rows = fetch_all("SELECT * FROM notifications ORDER BY id DESC LIMIT 500")
        df = pd.DataFrame(rows)
        st.dataframe(df, use_container_width=True, hide_index=True)
        ...
        if not df.empty:
            st.download_button(
                "알림 로그 CSV 다운로드",
                data=df.to_csv(index=False).encode("utf-8-sig"),
                file_name=f"notifications_backup_{date.today().isoformat()}.csv",
                mime="text/csv",
                use_container_width=True,
            )
